...
for name in \
	...
	SERVER_AUTH_REFRESH_COOKIE_PATH; do
	require_env "${name}"
done
...
service_url=""
runtime_env_vars="NODE_ENV=production,HOST=0.0.0.0,GRAPHQL_PATH=/graphql,PUBSUB_DRIVER=redis,DATABASE_SYNCHRONIZE=false,DATABASE_SSL=true,DATABASE_SSL_REJECT_UNAUTHORIZED=false,CORS_ORIGIN=${CLOUD_RUN_CORS_ORIGIN},AUTH_REFRESH_TOKEN_TRANSPORT=${SERVER_AUTH_REFRESH_TOKEN_TRANSPORT},AUTH_REFRESH_COOKIE_SECURE=${SERVER_AUTH_REFRESH_COOKIE_SECURE},AUTH_REFRESH_COOKIE_SAME_SITE=${SERVER_AUTH_REFRESH_COOKIE_SAME_SITE},AUTH_REFRESH_COOKIE_PATH=${SERVER_AUTH_REFRESH_COOKIE_PATH},LOG_VERBOSE_PUBSUB=false,LOG_GRAPHQL_SUBSCRIPTIONS=false"
