...
async function main() {
  ...
  if (!databaseName) {
    throw new Error('Could not determine database name from DATABASE_URL_DIRECT');
  }
  ...
  try {
    ...
    if (existingRole.rowCount > 0) {
      console.error(`Rotating password for existing Neon app role "${roleName}".`);
    ...
    await adminClient.query(`GRANT USAGE ON SCHEMA public TO ${roleIdentifier}`);
    await adminClient.query(
