...
/******/ (() => {
  ...
  \**************************************************************/
      ...
      /* harmony export */ __webpack_require__.d(__webpack_exports__, {
        ...
        /* harmony export */ escapeArgumentIfNeeded: () =>
          /* binding */ escapeArgumentIfNeeded,
        /* harmony export */
      });
      // Copyright (c) Microsoft Corporation. All rights reserved. Licensed under the MIT license.
  ...
  \**********************************************************/
       ...
       *   nameString:-fallbackString -> group 1: nameString,    group 2: fallbackString
       */
      const ENV_VAR_WITH_FALLBACK_REGEX =
        /^(?<name>[^:-]+)(?::?-(?<fallback>.+))?$/;
      /**
       *
            ...
            if (filterNpmIncompatibleProperties) {
              ...
              if (match) {
                ...
                if (isRegistryScoped) {
                  ...
                  if (lastColonIndex !== -1) {
                    const registryPropertySuffix = propertyName.substring(
                      lastColonIndex + 1,
                    );
                    if (
                      NPM_INCOMPATIBLE_REGISTRY_SCOPED_PROPERTIES.has(
                        registryPropertySuffix,
                      )
                    ) {
  ...
  \*****************************************************/
    ...
    /* harmony import */ var _utilities_npmrcUtilities__WEBPACK_IMPORTED_MODULE_4__ =
      __webpack_require__(/*! ../utilities/npmrcUtilities */ 359480);
    /* harmony import */ var _utilities_executionUtilities__WEBPACK_IMPORTED_MODULE_5__ =
      __webpack_require__(/*! ../utilities/executionUtilities */ 953844);
    // Copyright (c) Microsoft Corporation. All rights reserved. Licensed under the MIT license.
    // See LICENSE in the project root for license information.
    ...
    function getNpmPath() {
      if (!_npmPath) {
        try {
          if (
            _utilities_executionUtilities__WEBPACK_IMPORTED_MODULE_5__.IS_WINDOWS
          ) {
            // We're on Windows
    ...
    function _resolvePackageVersion(
      ...
      if (version.match(/^[a-zA-Z0-9\-\+\.]+$/)) {
        ...
        try {
          ...
          const npmVersionSpawnResult = _runNpmConfirmSuccess(
            [
              "view",
              `${name}@${version}`,
              "version",
    ...
    function _buildShellCommand(command, args) {
      ...
      _utilities_executionUtilities__WEBPACK_IMPORTED_MODULE_5__.escapeArgumentIfNeeded)(
        command,
      );
    ...
    function _runNpmConfirmSuccess(args, options, commandNameForLogging) {
      ...
      if (
        ...
        result = node_child_process__WEBPACK_IMPORTED_MODULE_0__.spawnSync(
          _buildShellCommand(command, args),
          {
            ...options,
            shell: true,
      ...
      if (result.status !== 0) {
        if (!result.status) {
          ...
          if (result.error) {
            throw new Error(
              `"${commandNameForLogging}" failed: ${result.error.message.toString()}`,
            );
          } else if (result.signal) {
            throw new Error(
              `"${commandNameForLogging}" was terminated by signal: ${result.signal}`,
            );
          } else {
            throw new Error(
              `"${commandNameForLogging}" failed for an unknown reason`,
            );
          }
        } else {
          throw new Error(
            `"${commandNameForLogging}" returned error code ${result.status}`,
          );
        }
      }
      return result;
    ...
    function installAndRun(
      ...
      try {
        ...
        if (
          ...
          result = node_child_process__WEBPACK_IMPORTED_MODULE_0__.spawnSync(
            _buildShellCommand(binPath, packageBinArgs),
            {
              ...spawnOptions,
              windowsVerbatimArguments: false,
    ...
    function _run() {
      ...
      const scriptFileName = node_path__WEBPACK_IMPORTED_MODULE_3__
        .basename(scriptPath)
        .toLowerCase();
      if (
        scriptFileName !== "install-run.js" &&
        scriptFileName !== "install-run"
      ) {
