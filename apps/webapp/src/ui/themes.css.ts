...
type SharedThemeValues = Pick<ThemeValues, "font" | "space" | "radius">;
type ThemeSpecificValues = Omit<ThemeValues, keyof SharedThemeValues>;
...
} satisfies Record<string, { label: string; values: ThemeSpecificValues }>;

const appThemes = defineWebappThemes({
  createThemeClass: (themeValues: ThemeValues) =>
    createTheme(vars, themeValues),
  defaultThemeName: "light",
  sharedValues: commonThemeValues,
  themes: appThemeDefinitions,
});

export const themeNames = appThemes.themeNames;
export type ThemeName = (typeof themeNames)[number];
export const themeLabelByName = appThemes.themeLabelByName;
export const defaultThemeName = appThemes.defaultThemeName;
export const themeClassByName = appThemes.themeClassByName;
