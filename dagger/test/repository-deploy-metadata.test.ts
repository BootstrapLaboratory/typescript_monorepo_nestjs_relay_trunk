...
import { servicesMeshPath, targetDefinitionPath } from "../src/deploy/metadata-paths.ts"
...
test("loads every target YAML referenced by the real services mesh", async () => {
  ...
  const loadedTargets = await Promise.all(
    Object.keys(mesh.services)
      .sort()
      .map(async (target) => ({
        definition: parseDeployTarget(await readFile(path.join(repoRoot, targetDefinitionPath(target)), "utf8")),
        target,
      })),
  )

  for (const { definition, target } of loadedTargets) {
    assert.equal(definition.name, target, `target definition "${target}" must declare a matching name`)
  }
})
...
test("every real services mesh target has a deploy script that exists on disk", async () => {
  ...
  await Promise.all(
    Object.keys(mesh.services).map(async (target) => {
      const definition = parseDeployTarget(await readFile(path.join(repoRoot, targetDefinitionPath(target)), "utf8"))
