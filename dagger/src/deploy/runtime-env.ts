...
function validateRepoRelativePath(path: string, target: string, name: string): string {
  const normalized = normalizeRepoRelativePath(path)

  if (normalized.length === 0 || normalized === ".") {
    throw new Error(`File mount source "${name}" for target "${target}" must resolve to a repository-relative file path.`)
  }
  ...
  if (segments.some((segment) => segment === "..")) {
    throw new Error(`File mount source "${name}" for target "${target}" must stay under the checked-out repository.`)
  }
